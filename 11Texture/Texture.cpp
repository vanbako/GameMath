	...
	, mpPixelData{ nullptr }
{
	...
	IWICBitmapFrameDecode* pBitmapSource{ nullptr };
	hr = pBitmapDecoder->GetFrame(0, &pBitmapSource);
	if (!SUCCEEDED(hr)) {
		SafeRelease(&pBitmapDecoder);
		SafeRelease(&pFactory);
	...
	hr = pBitmapSource->GetSize(&mWidth, &mHeight);
	if (!SUCCEEDED(hr) || mWidth == 0 || mHeight == 0) {
		SafeRelease(&pBitmapSource);
		SafeRelease(&pBitmapDecoder);
		SafeRelease(&pFactory);
		return;
	...
	mpPixelData = new (std::nothrow) BYTE[mWidth * mHeight * 3];
	...
	WICRect wicRect{ 0, 0, static_cast<INT>(mWidth), static_cast<INT>(mHeight) };
	hr = pBitmapSource->CopyPixels(&wicRect, mWidth * 3, mWidth * mHeight * 3, mpPixelData);
	if (!SUCCEEDED(hr)) {
		delete[] mpPixelData;
		mpPixelData = nullptr;
		SafeRelease(&pBitmapSource);
