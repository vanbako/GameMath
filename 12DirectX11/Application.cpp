...
void gm::Application::Input(const double duration)
{
	...
	if (mCursorMode == gm::CursorMode::Relative)
		if (mMoveCamera)
			...
			for (auto pMesh : mpMeshes)
				pMesh->Move(float(duration), mouseButton, mCursorPos);
...
void gm::Application::Render()
{
	...
	//for (auto pMesh : mpMeshes)
	//	pMesh->Render(&mWindow, pSolidColorBrush);
	//
